...
// JPA 사용 시, 대상이 되는 테이블 내부에 pk가 정수형태라면? 
//  -> <> : 제네릭 기법을 사용하기 때문에, 정수형태도 int의 객체 형태인 
...
public interface DataMemberRepository extends JpaRepository<DataMember, String> {
	...
	//*** 여러 개 단어를 하나로 작성할 떄, 반드시 카멜식 기법 따라야 한다.
	// 3. save(DateMember m) 
	// : 데이터를 추가(insert) / 데이터를 수정(update)
	// *** id 값을 기준으로 봤을 때, 데이터가 존재한다면 업데이트, 존재하지 않는다면 insert 수행
	// 4. deleter(String id)
	...
	// By : 조건을 부여 (where)
	//테이블의 컬럼이름들을 카멜식으로 표현 (And, Or Between a And b)
	// 조건절에 들어오는 매개변수를 낱개로 개수를 매칭해서 받아와야 함
	
	// ex1 ) id와 비밀번호가 일치한 데이터를 조회 
